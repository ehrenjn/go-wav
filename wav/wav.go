...
func ReadReader(r io.ReadCloser) (*Wav, error) { // creates a Wav from the ReadCloser
	buffer := make([]byte, 8)
	r.Read(buffer)
	...
	r.Read(dataBuffer) //dataBuffer IS EVERYTHING AFTER THE FIRST 8 BYTES
	r.Close()
	...
	return Create(dataBuffer[36:], channels, samplerate, bitDepth), nil //first 36 bytes is still meta data!
}
